...
class DraftAppAssetsDownloader(AsyncSandboxInitializer):
    ...
    To avoid the Linux ``ARG_MAX`` limit ("Argument list too long"), the
    script is transmitted via **stdin** to ``bash -s`` instead of being
    passed as a ``-c`` command-line argument.
    """
    ...
    def initialize(self, sandbox: Sandbox, ctx: SandboxInitializeContext) -> None:
        ...
        with with_connection(sandbox.vm) as conn:
            sandbox_root = sandbox.vm.get_working_path()
            ...
            future._stdin_transport.write(script_bytes + b"\n")
            future._stdin_transport.close()

            cmd_result = future.result(timeout=self._TIMEOUT)

        exit_code = cmd_result.exit_code if cmd_result.exit_code is not None else -1

        if exit_code != 0:
            stderr_text = cmd_result.stderr.decode("utf-8", errors="replace")[:500] if cmd_result.stderr else ""
            logger.error(
                "Draft assets download failed (exit_code=%d, stderr=%s)",
                exit_code,
                stderr_text,
            )
            return
